...
export const Decrypt = ({
  ...
  decryptAmountNum: number;
  decryptionStep: EncryptionStep;
  error: Error | null;
}) => {
  return (
      ...
      <div className="grid grid-cols-2 gap-8 relative">
        ...
        <div className="space-y-2 text-left flex flex-col justify-center text-sm">
          ...
          <p className="text-muted-foreground text-center">
            Public:{" "}
            {(formattedBalance + decryptAmountNum).toFixed(asset.roundTo ?? 2)}{" "}
            {asset.symbol}
          </p>
          <p className="text-muted-foreground text-center">
            Private:{" "}
            {(formattedPrivateBalance - decryptAmountNum).toFixed(
              asset.roundTo ?? 2,
            )}{" "}
            {asset.symbol}
          </p>
        ...
        <div className="space-y-2 text-left text-xs">
          <StepIndicator
            label="1. Generate Zero Knowledge proof of withdrawal"
            step="proof-generation"
            currentStep={decryptionStep}
            error={error}
          />
          <StepIndicator
            label="2. Call withdraw() on commbank.eth's PrivateUnstoppableMoney contract"
            step="deposit"
            currentStep={decryptionStep}
            error={error}
          />
