...
async function getLatestIPFSHash(): Promise<string> {
  ...
  try {
    ...
    throw new Error(`Could not read IPFS hash from ${hashFile}. Run 'npm run build' first.`);
  }
}

async function validateENSOwnership(signer: ethers.Signer, ensNode: string, domain: string): Promise<void> {
  console.log(`🔍 Validating ownership of ${domain}...`);
  
  const ensRegistry = new ethers.Contract(ENS_REGISTRY_ADDRESS, ENS_REGISTRY_ABI, signer);
...
async function updateContentHash(
  ...
  try {
    ...
    if (resolverAddress === ethers.ZeroAddress) {
      ...
      const setResolverTx = await ensRegistry.setResolver(ensNode, ENS_PUBLIC_RESOLVER_ADDRESS);
      await setResolverTx.wait();
      console.log('✅ Public resolver set');
    }
    
    // Use the resolver (either existing or newly set)
    const finalResolver = resolverAddress === ethers.ZeroAddress ? ENS_PUBLIC_RESOLVER_ADDRESS : resolverAddress;
    const resolver = new ethers.Contract(finalResolver, ENS_RESOLVER_ABI, signer);
    ...
    if (receipt?.status === 1) {
      ...
      console.log(`🌐 Your site should now be accessible at: https://commbank.eth`);
