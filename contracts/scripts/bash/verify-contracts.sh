...
# Run from the contracts/ directory with: bash scripts/bash/verify-sepolia-contracts.sh [--chain CHAIN_ID]
# Example: bash scripts/bash/verify-sepolia-contracts.sh --chain 11155111
...
npx hardhat verify --network "$NETWORK" "$COMMBANK_DOT_ETH" "$DEPOSIT_VERIFIER" "$TRANSFER_VERIFIER" "$WITHDRAW_VERIFIER"
